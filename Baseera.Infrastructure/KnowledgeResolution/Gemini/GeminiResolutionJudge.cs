...
public sealed class GeminiResolutionJudge : IResolutionJudge
{
        ...
        IReadOnlyList<ResolutionCandidate> candidates)
    {
        ...
        12. If the extracted entity name directly refers to the
            candidate's canonical name, use "ExactMatch".

        13. If the extracted entity refers to the same real-world
            entity but the extracted name is a different name,
            abbreviation, spelling variation, or alternative name,
            use "AliasMatch".

        14. For "Match", resolutionType must be either
            "ExactMatch" or "AliasMatch".

        15. For "NoMatch" or "Ambiguous",
            resolutionType must be null.
        ...
        IReadOnlyList<ResolutionCandidate> candidates)
    {
        ...
        if (decision == ResolutionDecisionType.Match)
        {
            ...
            if (!Enum.TryParse<ResolutionType>(
                    geminiDecision.ResolutionType,
                    ignoreCase: true,
                    out var parsedResolutionType))
            {
                throw new InvalidOperationException(
                    "Gemini returned Match without a valid resolution type.");
            }

            candidateId = parsedId;
            resolutionType = parsedResolutionType;
        }
        else
        {
            ...
            {
                throw new InvalidOperationException(
                    $"{decision} decision must not contain a candidate ID.");
            }
            ...
            {
                throw new InvalidOperationException(
                    $"{decision} decision must not contain a resolution type.");
            }
        }
